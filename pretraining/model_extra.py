...
class Bert(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, masked_lm_labels, num_masked=None, ratio=None,return_all=False):
        contextualized_embeddings = self.get_contextualized(input_ids, attention_mask)

        if num_masked is None:

              
            subword_prediction = self.classifier(contextualized_embeddings, masked_lm_labels, num_masked,return_all)
            ...
            if return_all:
                ...
                return subword_prediction,loss, accuracy, z_loss, num_tokens #added prediction
        else:
            print("hello")
...
class MaskClassifier(nn.Module):
    ...
    def forward(self, x, masked_lm_labels, num_masked=None,return_all=False):
        if num_masked is None:
            print(f"return_all value: {return_all}")
            if return_all==True:
                print(x.shape)
                x = x.flatten(0,1)
                ...
                print(f" x in beginning : {x.shape}")
                x = torch.index_select(x.flatten(0, 1), 0, torch.nonzero(masked_lm_labels.flatten() != -100).squeeze())
                print(f" x after flatten : {x.shape}")
                x = self.nonlinearity(x)
                print(f" x after non linearity: {x.shape}")
                return x
        else:
