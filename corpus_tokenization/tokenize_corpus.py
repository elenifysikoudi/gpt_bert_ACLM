# in the input directory, output directory, path to the tokenizer, and the max sequence length
...
def parse_args():
    ...
    parser.add_argument("--data_folder", type=Path, default="../data/processed")
    parser.add_argument("--train_file", type=Path, default="cached_128.txt")
    parser.add_argument("--valid_file", type=Path, default="../data/processed/dev/cached_128.txt")
    parser.add_argument("--tokenizer_folder", type=Path, default="/mimer/NOBACKUP/groups/naiss2024-6-297/gpt-bert/tokenizers")
    parser.add_argument("--tokenizer_file", type=Path, default="tokenizer_10M_new.json")
...
if __name__ == "__main__":
    ...
    output_train_path = "/mimer/NOBACKUP/groups/naiss2024-6-297/gpt-bert/data/train_10M_tokenized_new.bin"

    #tokenize_dataset(full_ds, output_train_path, tokenizer)

    tokenize_file(input_train_path, output_train_path, tokenizer)

    output_valid_path = "/mimer/NOBACKUP/groups/naiss2024-6-297/gpt-bert/data/dev_10M_tokenized_new.bin"
    
    #tokenize_dataset(valid_ds, output_valid_path, tokenizer)
    if args.valid_file is not None:
        input_valid_path = args.valid_file
